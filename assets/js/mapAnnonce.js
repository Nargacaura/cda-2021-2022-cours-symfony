            ...
            xhr.then(response => response.json())
                .then(json => {
                    json.forEach(element => {
                        ...
                        // console.log(element)
                        var pinpoint_annonce = L.marker([element.latitude, element.longitude], {icon: icon_annonce}).addTo(annoncesInMap).bindPopup("<b>" + element.annonce[0].title + "</b> (" + element.annonce[0].price + "€)<br>" + element.annonce[0].description + "<br><a href='/annonce/" + element.annonce[0].id + "'>Voir l'annonce</a>")
                    })
                })
        }
